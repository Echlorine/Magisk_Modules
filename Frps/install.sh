...
on_install() {
  ...
  tar zxvf $MODPATH/frp.tar.gz frp_${suffix}_linux_arm64/frps --strip-components 1 -C $MODPATH/
...
set_permissions() {
  ...
  set_perm $MODPATH/frps 0 0 0755
  set_perm $MODPATH/frps.log 00 0664

  ui_print "✓ Now no need to reboot..."
  ui_print "+ Attempting to start Frps"
  ui_print "+ please wait..."
  ui_print ""

  if [[ -e "$CONFIG_PATH/frps.toml" ]]; then
      cp -af $CONFIG_PATH/frps.toml $MODPATH/frps.toml
      sed -i "s|log.to.*|log.to = \"${MODPATH}/frps.log\"|g" "$MODPATH/frps.toml"
      set_perm $MODPATH/frps.toml 00 0644
      $MODPATH/frps -c $MODPATH/frps.toml > /dev/null 2>&1 &
      sleep 5
      if [[ -e "$MODPATH/frps.toml" ]]; then
        cat "$MODPATH"/frps.log
    ...
    ui_print "$CONFIG_PATH/frps.toml 配置文件不存在"
    ui_print ""
    ui_print "[!] 请确认配置文件后重新启动"
  fi
}
